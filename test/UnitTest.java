...
public class UnitTest implements JavaHelpers {
    ...
    public void checkIndex() {
        ...
        final AuthorController controller = new AuthorController(formFactory, repository, ec);
    ...
    public void checkAddAuthor() {
        ...
        author.id = 1L;
        author.shortName = "Muro, F";
        author.fullName = "Muro, Francisco";
        author.affiliation = "CEU";
        when(repository.add(any())).thenReturn(supplyAsync(() -> author));

        // Set up the request builder to reflect input
        final Http.RequestBuilder requestBuilder = new Http.RequestBuilder().method("post").bodyJson(Json.toJson(author));
